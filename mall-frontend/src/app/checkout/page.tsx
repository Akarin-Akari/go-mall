...
import { CartItem, Address, CreateOrderRequest } from '@/types';
...
const CheckoutPage: React.FC = () => {
  ...
  }, [selectedAddress, agreementChecked, selectedItems, shippingMethod, paymentMethod, buyerMessage, dispatch, router]);
    ...
    return (
      <Card title="订单摘要" style={{ marginTop: 16 }}>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Text>运费</Text>
            <Text>{parseFloat(orderSummary.shippingFee) === 0 ? '免运费' : `¥${orderSummary.shippingFee}`}</Text>
          </div>
          <Divider />
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Title level={4}>实付款</Title>
            <Title level={4} style={{ color: '#ff4d4f' }}>¥{orderSummary.total}</Title>
          </div>
        </Space>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            我已阅读并同意 <a href="/agreement" target="_blank">《服务协议》</a> 和 <a href="/privacy" target="_blank">《隐私政策》</a>
          </Checkbox>
        </div>
        ...
        <Button
          ...
          disabled={!selectedAddress || !agreementChecked || selectedItems.length === 0}
        >
          提交订单
        </Button>
      </Card>
