...
import { fetchProductDetailAsync, selectProduct } from '@/store/slices/productSlice';
...
const ProductDetailPage: React.FC = () => {
  ...
  const handleAddToCart = useCallback(async () => {
    ...
    try {
      ...
      // 检查规格选择
      const requiredSpecs = productSpecs.filter(spec => spec.options.length > 1);
      const missingSpecs = requiredSpecs.filter(spec => !selectedSpecs[spec.name]);
      
      if (missingSpecs.length > 0) {
        message.warning(`请选择${missingSpecs.map(spec => spec.name).join('、')}`);
        return;
      }
      ...
      <div style={{ padding: '0 24px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <Row gutter={[32, 32]}>
          ...
          <Col xs={24} md={12} lg={10}>
            <ProductImageGallery 
              images={currentProduct.images || ['/images/product-placeholder.svg']}
            ...
            <div style={{ position: 'sticky', top: 80 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <Space wrap style={{ marginBottom: 8 }}>
                  {currentProduct.is_hot && <Tag color="red">热销</Tag>}
                  {currentProduct.is_new && <Tag color="green">新品</Tag>}
                  {currentProduct.is_recommend && <Tag color="blue">推荐</Tag>}
                ...
                <Space split={<Divider type="vertical" />}>
                  <Space>
                    <Rate 
                      ...
                      style={{ fontSize: 16 }}
                    />
                    <Text strong>{currentProduct.rating?.toFixed(1) || '暂无评分'}</Text>
                  </Space>
                  <Text type="secondary">
              ...
              <Card style={{ marginBottom: 24, backgroundColor: '#fafafa' }}>
                <Space direction="vertical" size="small" style={{ width: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 16 }}>
                    <Text style={{ fontSize: 32, color: '#ff4d4f', fontWeight: 'bold' }}>
                      ¥{formatPrice(currentProduct.discount_price || currentProduct.price)}
                    </Text>
                    {currentProduct.discount_price && parseFloat(currentProduct.discount_price) < parseFloat(currentProduct.price) && (
                  ...
                  {currentProduct.discount_price && parseFloat(currentProduct.discount_price) < parseFloat(currentProduct.price) && (
                    <div>
                      <Tag color="red" style={{ fontSize: 14 }}>
                        立省 ¥{formatPrice((parseFloat(currentProduct.price) - parseFloat(currentProduct.discount_price)).toString())}
                      </Tag>
                      <Text type="secondary" style={{ marginLeft: 8 }}>
                        折扣 {((parseFloat(currentProduct.discount_price) / parseFloat(currentProduct.price)) * 100).toFixed(0)}%
                      </Text>
                    </div>
                  )}
                </Space>
              ...
              <Card size="small" style={{ backgroundColor: '#f9f9f9' }}>
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <Space wrap>
                    <Tag icon={<TruckOutlined />} color="blue">免费配送</Tag>
                    <Tag icon={<SafetyCertificateOutlined />} color="green">正品保证</Tag>
                    <Tag icon={<CustomerServiceOutlined />} color="orange">7天退换</Tag>
                    <Tag icon={<CheckCircleOutlined />} color="purple">售后保修</Tag>
                  </Space>
                </Space>
              </Card>
            </div>
              ...
              <div style={{ padding: '24px 0' }}>
                ...
                <Row gutter={[16, 16]}>
                  ...
                  <Col span={12}>
                    ...
                    <Text>#{currentProduct.id.toString().padStart(8, '0')}</Text>
                  </Col>
                  <Col span={12}>
                ...
                <Row gutter={[32, 32]} style={{ marginBottom: 32 }}>
                  <Col xs={24} md={8}>
                    <Card style={{ textAlign: 'center', backgroundColor: '#fafafa' }}>
                      <div style={{ fontSize: 48, color: '#ff4d4f', fontWeight: 'bold' }}>
                      ...
                      />
                      <div style={{ color: '#666' }}>
                        基于 {formatNumber(currentProduct.sales_count || 0)} 条评价
                      </div>
                    </Card>
                  </Col>
                  <Col xs={24} md={16}>
                    <Space direction="vertical" style={{ width: '100%' }} size="middle">
                      {[5, 4, 3, 2, 1].map(star => (
                        <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                          ...
                          <Progress
                            percent={star === 5 ? 80 : star === 4 ? 15 : star === 3 ? 3 : star === 2 ? 1 : 1}
                            strokeColor="#1890ff"
                            style={{ flex: 1 }}
                          />
                    ...
                    {[
                      {
                        ...
                        rating: 5,
                        content: '商品质量很好，物流也很快，非常满意的一次购物体验！',
                        images: [],
                        date: '2024-01-20',
                      ...
                      },
                      {
                        ...
                        rating: 4,
                        content: '整体不错，性价比很高，推荐购买。包装也很精美。',
                        images: [],
                        date: '2024-01-18',
                    ...
                    <Card>
                      <Space direction="vertical" style={{ width: '100%' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <div>
                            <Title level={5} style={{ margin: 0 }}>免费配送</Title>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <div>
                            <Title level={5} style={{ margin: 0 }}>正品保证</Title>
                            <Text type="secondary">官方授权，假一赔十</Text>
                          </div>
                        </div>
                        <Divider />
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <CustomerServiceOutlined style={{ fontSize: 24, color: '#fa8c16' }} />
                          <div>
                            <Title level={5} style={{ margin: 0 }}>7天退换</Title>
